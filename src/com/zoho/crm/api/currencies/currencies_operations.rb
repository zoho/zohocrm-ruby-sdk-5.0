  ...
  module Currencies
    class CurrenciesOperations
      ...
      def update_currencies(request)
        ...
        # @param currency [Integer] A Integer
        # @return An instance of APIResponse
      # @raise SDKException
      def get_currency(currency)
        if !currency.is_a? Integer
          raise SDKException.new(Constants::DATA_TYPE_ERROR, 'KEY: currency EXPECTED TYPE: Integer', nil, nil)
        end
        handler_instance = Handler::CommonAPIHandler.new
        api_path = ''
        api_path = api_path + '/crm/v5/org/currencies/'
        api_path = api_path + currency.to_s
        handler_instance.api_path = api_path
        handler_instance.http_method = Constants::REQUEST_METHOD_GET
        handler_instance.category_method = 'READ'
        require_relative 'response_handler'
        ...
        # @return An instance of APIResponse
      # @raise SDKException
      def update_currency(currency, request)
        if !currency.is_a? Integer
          raise SDKException.new(Constants::DATA_TYPE_ERROR, 'KEY: currency EXPECTED TYPE: Integer', nil, nil)
        end
        if request!=nil and !request.is_a? BodyWrapper
