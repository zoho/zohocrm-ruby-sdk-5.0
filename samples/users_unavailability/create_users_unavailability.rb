...
class CreateUsersUnavailability
		def self.create_users_unavailability
			...
			from = DateTime.new(2023, 10, 02, 11, 00, 01, "+05:30")
			user1.from = from
			to = DateTime.new(2023, 10, 02, 11, 00, 01, "+05:30")
			user1.to = to
			user =  ZOHOCRMSDK::UsersUnavailability::User.new
			user.id = 440248001305249
			user1.user = user
			user_list.push(user1)
			request.users_unavailability = user_list
			response = users_operations.create_users_unavailability(request)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				create_users_unavailability
			rescue Exception => e
				raise e
			end
		end
end
CreateUsersUnavailability.initialize_and_call