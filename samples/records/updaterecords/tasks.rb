...
class Tasks
		def self.update_task(module_api_name)
			...
			record1.id = 303453453467895
			record1.add_field_value(ZOHOCRMSDK::Record::Field::Tasks.Description, "test Task")
			record1.add_key_value("rupee",  ZOHOCRMSDK::Util::Choice.new("inr"))
			remind_at =  ZOHOCRMSDK::Record::RemindAt.new
			remind_at.alarm="ACTION=EMAILANDPOPUPTRIGGER=DATE-TIME:2023-07-03T12:30:00+05:30"
			record1.add_field_value(ZOHOCRMSDK::Record::Field::Tasks.Remind_at, remind_at)
			#			who_id=Record.new
			#		who_id.id = 440248774074)
			#		record1.add_field_value(ZOHOCRMSDK::Record::Field::Tasks.Who_id, who_id)
			record1.add_field_value(ZOHOCRMSDK::Record::Field::Tasks.Status,  ZOHOCRMSDK::Util::Choice.new("not Started"))
			record1.add_field_value(ZOHOCRMSDK::Record::Field::Tasks.Due_date, Date.new(2023, 12, 13))
			record1.add_field_value(ZOHOCRMSDK::Record::Field::Tasks.Priority,  ZOHOCRMSDK::Util::Choice.new("low"))
			record1.add_field_value(ZOHOCRMSDK::Record::Field::Tasks.Subject, "email1")
			record1.add_field_value(ZOHOCRMSDK::Record::Field::Tasks.Send_notification_email, false)
			record1.add_key_value("$se_module", "leads")
			what_id=ZOHOCRMSDK::Record::Record.new
			what_id.id = 440248774074
			...
			recurring_activity.rrule="FREQ=DAILYINTERVAL=10UNTIL=2023-08-14DTSTART=2023-07-03"
	...
	def self.initialize_and_call
			begin
				...
				update_task(module_api_name)
			rescue Exception => e
				raise e
			end
		end
end
Tasks.initialize_and_call