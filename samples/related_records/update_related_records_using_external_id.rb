...
class UpdateRelatedRecordsUsingExternalId
		def self.update_related_records_using_external_id(module_api_name, external_value, related_list_api_name)
		 ...
		 # Call addKeyValue method that takes two arguments 1 -> A string that is the Field's API Name 2 -> Value
			record1.add_key_value("products__external", "automated_sdk_external")
			record1.add_key_value("list_price", 50.56)
			records.push(record1)
			record2=ZOHOCRMSDK::Record::Record.new
		 # Call addKeyValue method that takes two arguments 1 -> A string that is the Field's API Name 2 -> Value
			record2.add_key_value("products__external", "automated_sdk_external")
	...
	def self.initialize_and_call
			begin
				...
				update_related_records_using_external_id(module_api_name, external_value, related_list_api_name)
			rescue Exception => e
				raise e
			end
		end
