...
class CreateMapDependency
		def self.create_map_dependency(layout_id, module_1)
			...
			parent.id = 3477061002611
			mapdependency.parent = parent
			child =  ZOHOCRMSDK::FieldMapDependency::Child.new
			child.api_name="Lead_Source"
			child.id = 347706116628001
			...
			pick_list_value.id = 3477061003409
			pick_list_value.actual_value="-None-"
			picklist_maps=[]
			picklist_map =  ZOHOCRMSDK::FieldMapDependency::PicklistMap.new
			picklist_map.id = 347706116628005
			picklist_map.actual_value="Option 1"
			picklist_map.display_value="Option 1"
			picklist_maps.push(picklist_map)
			picklist_map =  ZOHOCRMSDK::FieldMapDependency::PicklistMap.new
			picklist_map.id = 347706116628003
	...
	def self.initialize_and_call
			begin
				...
				create_map_dependency(layout_id, module_api_name)
			rescue Exception => e
				raise e
			end
		end
end
CreateMapDependency.initialize_and_call