...
class GetMapDependencies
		def self.get_map_dependencies(layout_id, module_1)
				...
				if response.is_expected
					...
					if response_handler.is_a? ZOHOCRMSDK::FieldMapDependency::BodyWrapper
						...
						map_dependencies.each do |map_dependency|
							...
							if parent != nil
								print "map_dependency Parent ID: " + parent.id.to_s + "\n"
								print "map_dependency Parent APIName: " + parent.api_name.to_s + "\n"
							end
							...
							print "map_dependency Source: " + map_dependency.source.to_s + "\n"
						end
						info = response_wrapper.info
						if info != nil
							if info.per_page != nil
								print "map_dependency Info PerPage: " + info.per_page.to_s + "\n"
							end
							if info.count != nil
								print "map_dependency Info Count: " + info.count.to_s + "\n"
							end
							if info.page != nil
								print "map_dependency Info Page: " + info.page.to_s + "\n"
							end
							if info.more_records != nil
								print "map_dependency Info MoreRecords: " + info.more_records.to_s + "\n"
							end
						end
	...
	def self.initialize_and_call
			begin
				...
				get_map_dependencies(layout_id, module_api_name)
			rescue Exception => e
				raise e
			end
		end
end
GetMapDependencies.initialize_and_call