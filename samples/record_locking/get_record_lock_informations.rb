...
class GetRecordLockInformations
		def self.get_record_locking_informations(module_api_name, record_id)
			record_locking_operations =  ZOHOCRMSDK::RecordLocking::RecordLockingOperations.new(record_id, module_api_name)
			param_instance =  ZOHOCRMSDK::ParameterMap.new
			param_instance.add(ZOHOCRMSDK::RecordLocking::RecordLockingOperations::GetRecordLockingInformationsParam.fields, "Lock_Source__s")
			response = record_locking_operations.get_record_locking_informations(param_instance)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				record_id = 6653898010
				get_record_locking_informations(module_api_name, record_id)
			rescue Exception => e
				raise e
			end
		end
end
GetRecordLockInformations.initialize_and_call