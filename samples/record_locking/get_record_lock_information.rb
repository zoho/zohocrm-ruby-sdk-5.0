...
class GetRecordLockInformation
		def self.get_record_lock_information(module_api_name, record_id)
			record_locking_operations =  ZOHOCRMSDK::RecordLocking::RecordLockingOperations.new
			param_instance =  ZOHOCRMSDK::ParameterMap.new
			param_instance.add(ZOHOCRMSDK::RecordLocking::RecordLockingOperations::GetrecordlockinformationParam.fields, "lock_source__s")
			response = record_locking_operations.get_record_lock_information(record_id, module_api_name, param_instance)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				record_id = 34770619001
				get_record_lock_information(module_api_name, record_id)
			rescue Exception => e
				raise e
			end
		end
