...
class DeleteAppointmentsRescheduledHistory
		def self.delete_appointments_rescheduled_history(id)
			reschedule_history_operations =  ZOHOCRMSDK::RescheduleHistory::RescheduleHistoryOperations.new
			response = reschedule_history_operations.delete_appointments_rescheduled_history(id)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				id = 3048374923478
				delete_appointments_rescheduled_history(id)
			rescue Exception => e
				raise e
			end
		end
end
DeleteAppointmentsRescheduledHistory.initialize_and_call