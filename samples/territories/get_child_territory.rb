...
class GetChildTerritory
		def self.get_child_territory(id)
			territories_operations =  ZOHOCRMSDK::Territories::TerritoriesOperations.new
			paraminstance =  ZOHOCRMSDK::ParameterMap.new
			paraminstance.add(ZOHOCRMSDK::Territories::TerritoriesOperations::GetChildTerritoryParam.filters, "")
			response = territories_operations.get_child_territory(id, paraminstance)
			if response != nil
				...
				if response.is_expected
					...
					if response_handler.is_a? ZOHOCRMSDK::Territories::ResponseWrapper
						...
						territory_list.each do |territory|
							...
							if deal_rule_criteria != nil
								print_criteria(deal_rule_criteria)
							end
	...
	def self.initialize_and_call
			begin
				...
				id = 440248001305227
				get_child_territory(id)
			rescue Exception => e
				raise e
			end
		end
end
GetChildTerritory.initialize_and_call