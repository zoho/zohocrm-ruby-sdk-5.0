...
class EnableMultipleCurrencies
		def self.enable_multiple_currencies
			...
			format.decimal_places =  ZOHOCRMSDK::Util::Choice.new("3")
			currency.format = format
			body_wrapper.base_currency = currency
			response = currencies_operations.enable_currency(body_wrapper)
			if response != nil
...
EnableMultipleCurrencies.initialize_and_call
