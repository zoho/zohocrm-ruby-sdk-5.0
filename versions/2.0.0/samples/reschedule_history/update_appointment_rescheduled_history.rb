...
class UpdateAppointmentRescheduledHistory
		def self.update_appointment_rescheduled_history(id)
			...
			reschedule_history.rescheduled_to = DateTime.new(2023, 11, 20, 10, 00, 01, "+05:30")
			reschedule_history.rescheduled_from = DateTime.new(2023, 11, 20, 11, 00, 01, "+05:30")
			reschedule_history.rescheduled_time = DateTime.new(2023, 11, 20, 10, 00, 01, "+05:30")
			...
			request.data = data
			response = reschedule_history_operations.update_appointment_rescheduled_history(id, request)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				id = 35687346127843
				update_appointment_rescheduled_history(id)
			rescue Exception => e
				raise e
			end
		end
end
UpdateAppointmentRescheduledHistory.initialize_and_call