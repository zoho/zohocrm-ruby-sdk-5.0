...
class GetAppointmentRescheduledHistory
		def self.get_appointment_rescheduled_history(id)
			reschedule_history_operations =  ZOHOCRMSDK::RescheduleHistory::RescheduleHistoryOperations.new
			param_instance =  ZOHOCRMSDK::ParameterMap.new
			response = reschedule_history_operations.get_appointment_rescheduled_history(id, param_instance)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				id = 3048374923478
				get_appointment_rescheduled_history(id)
			rescue Exception => e
				raise e
			end
		end
end
GetAppointmentRescheduledHistory.initialize_and_call