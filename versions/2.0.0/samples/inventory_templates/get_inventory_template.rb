...
class GetInventoryTemplate
		def self.get_inventory_template(inventory_template_id)
			inventory_templates_operations =  ZOHOCRMSDK::InventoryTemplates::InventoryTemplatesOperations.new
			response = inventory_templates_operations.get_inventory_template(inventory_template_id)
			if response != nil
				...
				if response.is_expected
					...
					if response_handler.is_a? ZOHOCRMSDK::InventoryTemplates::ResponseWrapper
						...
						inventory_templates.each do |inventory_template|
							...
							print "inventory_template : " + inventory_template.editor_mode.to_s + "\n"
	...
	def self.initialize_and_call
			begin
				...
				inventory_template_id = 4402480258279
				get_inventory_template(inventory_template_id)
			rescue Exception => e
				raise e
			end
		end
