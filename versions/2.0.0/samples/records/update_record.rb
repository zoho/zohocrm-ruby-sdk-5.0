...
class UpdateRecord
		def self.update_record(module_api_name, record_id)
		 ...
		 # Call addKeyValue method that takes two arguments 1 -> A string that is the Field's API Name 2 -> Value
			...
			record1.add_key_value("multiselect",  [Choice.new("option 1")])
			...
			deletesubform=ZOHOCRMSDK::Record::Record.new
			deletesubform.id = 34770619314010
			deletesubform.add_key_value("_delete", nil)
			subform_list.push(deletesubform)
	...
	#		inventory_line_item.add_key_value("parent__id", 5)
			...
			product_line_tax.name="Sales Tax"
	...
	def self.initialize_and_call
			begin
				...
				record_id = 4402481018118
				update_record(module_api_name, record_id)
			rescue Exception => e
				raise e
			end
		end
