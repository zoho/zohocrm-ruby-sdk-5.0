...
class SalesOrders
		def self.update_sales_orders(module_1)
	...
	#		account.add_key_value("name", "automated")
			...
			deal_name.add_field_value(ZOHOCRMSDK::Record::Field::Deals.id, 3212431324313)
			...
			contact_name.add_field_value(ZOHOCRMSDK::Record::Field::Contacts.id, 32343533322342)
			...
			inventory_line_item_list2=[]
			inventory_line_item2=ZOHOCRMSDK::Record::Record.new
			line_item_product2 =  ZOHOCRMSDK::Record::LineItemProduct.new
			line_item_product2.id = 440248954344
			#	data=Record.new
			#	data.id = 440248954344
			inventory_line_item2.add_key_value("description", "asd")
			inventory_line_item2.add_key_value("discount", "5")
			inventory_line_item2.add_key_value("quantity", 10.0)
			inventory_line_item2.add_key_value("list__price", 100.0)
			inventory_line_item2.add_key_value("product__name", line_item_product2)
			# inventory_line_item2.add_key_value("product__category_1", "hardware")
			inventory_line_item_list2.push(inventory_line_item2)
			record1.add_field_value(ZOHOCRMSDK::Record::Field::Sales_Orders.Ordered_items, inventory_line_item_list2)
			line_taxes=[]
	...
	def self.initialize_and_call
			begin
				...
				update_sales_orders(module_api_name)
			rescue Exception => e
				raise e
			end
		end
