...
class GetRecordUsingExternalId
		def self.get_record_using_external_id(module_api_name, external_field_value, destination_folder)
			...
			field_names.each do |field_name|
				param_instance.add(ZOHOCRMSDK::Record::RecordOperations::GetRecordParam.fields, field_name)
			end
					...
					if response_handler.is_a? ZOHOCRMSDK::Record::ResponseWrapper
						...
						records.each do |record|
							...
							record.each do |k, v|
								...
								elsif value.is_a? ZOHOCRMSDK::Record::Layouts
									layout= value
									if layout != nil
	...
	def self.initialize_and_call
			begin
				...
				get_record_using_external_id(module_api_name, external_field_value, destination_folder)
			rescue Exception => e
				raise e
			end
		end
