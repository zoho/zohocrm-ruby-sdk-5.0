...
class GetRecordLockInformation
		def self.get_record_lock_information(module_api_name, record_id, lock_id)
			record_locking_operations =  ZOHOCRMSDK::RecordLocking::RecordLockingOperations.new(record_id, module_api_name)
			param_instance =  ZOHOCRMSDK::ParameterMap.new
			param_instance.add(ZOHOCRMSDK::RecordLocking::RecordLockingOperations::GetRecordLockingInformationParam.fields, "Lock_Source__s")
			response = record_locking_operations.get_record_locking_information(lock_id, param_instance)
			if response != nil
				...
				if response.is_expected
					...
					if response_handler.is_a? ZOHOCRMSDK::RecordLocking::ResponseWrapper
						...
						record_locks.each do |record_lock|
							record_lock.get_key_values.each do |k, v|
								key_name = k
								value = v
	...
	def self.initialize_and_call
			begin
				...
				record_id = 665389388010
				lock_id = 6653890049040
				get_record_lock_information(module_api_name, record_id,lock_id)
			rescue Exception => e
				raise e
			end
		end
end
GetRecordLockInformation.initialize_and_call