...
class DeassociateTerritoryUsers 
		def self.deassociate_territory_users(territory)
			...
			paraminstance.add(ZOHOCRMSDK::TerritoryUsers::TerritoryUsersOperations::DeassociateTerritoryUsersParam.ids, "440248001305249")
			response = territory_users_operations.deassociate_territory_users(territory, paraminstance)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				deassociate_territory_users(territory)
			rescue Exception => e
				raise e
			end
		end
end
DeassociateTerritoryUsers.initialize_and_call