...
class UpdateTerritoryUsers 
		def self.update_territoryusers(territory)
			territory_users_operations =  ZOHOCRMSDK::TerritoryUsers::TerritoryUsersOperations.new
			request =  ZOHOCRMSDK::TerritoryUsers::BodyWrapper.new
			users=[]
			user =  ZOHOCRMSDK::Users::Users.new
			user.id = 332133213243242
			users.push(user)
			request.users = users
	...
	def self.initialize_and_call
			begin
				...
				territory_id = 347706114966002
				update_territoryusers(territory_id)
			rescue Exception => e
				raise e
			end
		end
end
UpdateTerritoryUsers.initialize_and_call