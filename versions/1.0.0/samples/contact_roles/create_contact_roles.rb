...
class CreateContactRoles
		def self.create_contact_roles
			...
			(1..2).each do |i|
				...
				contact_role.name ="contactrole23" + i.to_s
				contact_role.sequence_number = i
				contact_roles.push(contact_role)
			end
			body_wrapper.contact_roles = contact_roles
			response = contact_roles_operations.create_roles(body_wrapper)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				create_contact_roles
			rescue Exception => e
				raise e
			end
		end
end
CreateContactRoles.initialize_and_call
