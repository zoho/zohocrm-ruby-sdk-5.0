...
class ValidateBeforeTransferForAllTerritories
		def self.validate_before_transfer_for_all_territories(user_id)
			users_territories_operations =  ZOHOCRMSDK::UsersTerritories::UsersTerritoriesOperations.new
			response = users_territories_operations.validate_before_transfer_for_all_territories(user_id)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				user_id = 34770611709
				validate_before_transfer_for_all_territories(user_id)
			rescue Exception => e
				raise e
			end
		end
end
ValidateBeforeTransferForAllTerritories.initialize_and_call