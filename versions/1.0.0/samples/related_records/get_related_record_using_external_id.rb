...
class GetRelatedRecordUsingExternalId
		def self.get_related_record_using_external_id(module_api_name, external_value, related_list_api_name, external_field_value, destination_folder)
			...
			header_instance.add(ZOHOCRMSDK::RelatedRecords::RelatedRecordsOperations::GetRelatedRecordHeader.If_modified_since, startdatetime)
			response = related_records_operations.get_related_record_using_external_id(external_field_value, external_value, param_instance, header_instance)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				get_related_record_using_external_id(module_api_name, external_value, related_list_api_name, external_field_value, destination_folder)
			rescue Exception => e
				raise e
			end
		end
