...
class GetRelatedEmails
		def self.get_related_emails(module_api_name, id)
			email_templates_operations =  ZOHOCRMSDK::EmailRelatedRecords::EmailRelatedRecordsOperations.new(id, module_api_name)
			param_instance =  ZOHOCRMSDK::ParameterMap.new
			param_instance.add(ZOHOCRMSDK::EmailRelatedRecords::EmailRelatedRecordsOperations::GetEmailsRelatedRecordsParam.owner_id, 123)
			param_instance.add( ZOHOCRMSDK::Param.new("message_id", "string"), "2132")
			response = email_templates_operations.get_emails_related_records(param_instance)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				id = 4402480774074
				get_related_emails(module_api_name, id)
			rescue Exception => e
				raise e
			end
		end
end
GetRelatedEmails.initialize_and_call