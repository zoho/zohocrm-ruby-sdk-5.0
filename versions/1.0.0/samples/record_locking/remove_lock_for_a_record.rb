...
class RemoveLockForARecord
		def self.remove_lock_for_a_record(record_id, module_name, lock_id)
			record_locking_operations =  ZOHOCRMSDK::RecordLocking::RecordLockingOperations.new
			response = record_locking_operations.remove_lock_for_a_record(lock_id, record_id, module_name)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				record_id = 347709001
				lock_id = 3477062002
				remove_lock_for_a_record(record_id, module_api_name, lock_id)
			rescue Exception => e
				raise e
			end
		end
end
RemoveLockForARecord.initialize_and_call