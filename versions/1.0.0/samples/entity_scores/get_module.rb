...
class GetModule
		def self.get_module(record_id, module_1)
			entity_scores_operations =  ZOHOCRMSDK::EntityScores::EntityScoresOperations.new("positive_score")
			response = entity_scores_operations.get_module(record_id, module_1)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				record_id = 440248001177154
				module_1 = "leads"
				get_module(record_id, module_1)
			rescue Exception => e
				raise e
			end
		end
end
GetModule.initialize_and_call