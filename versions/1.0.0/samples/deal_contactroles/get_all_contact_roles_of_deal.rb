...
class GetAllContactRolesOfDeal
		def self.get_all_contact_roles_of_deal(deal_id)
			...
			# param_instance.add(ZOHOCRMSDK::DealContactRoles::DealContactRolesOperations::GetAssociatedContactRolesParam.ids, 347706114564040)
			param_instance.add(ZOHOCRMSDK::DealContactRoles::DealContactRolesOperations::GetAssociatedContactRolesParam.fields, "id")
				...
				if response.is_expected
					...
					if response_handler.is_a? ZOHOCRMSDK::DealContactRoles::BodyWrapper
						...
						if info != nil
							...
							if info.next_page_token != nil
								print "record Info PerPage: " + info.next_page_token.to_s + "\n"
							end
							if info.count != nil
							...
							if info.previous_page_token != nil
								print "record Info PreviousPageToken: " + info.previous_page_token.to_s + "\n"
							end
							if info.page_token_expiry != nil
								print "record Info PageTokenExpiry: " + info.page_token_expiry.to_s + "\n"
							end
	...
	def self.initialize_and_call
			begin
				...
				deal_id = 4402480262182
				get_all_contact_roles_of_deal(deal_id)
			rescue Exception => e
				raise e
			end
		end
end
GetAllContactRolesOfDeal.initialize_and_call